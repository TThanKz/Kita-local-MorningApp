Kita Morgen – Check-in V3
==========================
Lokaler iPad-Prototyp.

Enthalten:
- 3 konfigurierbare Gruppen
- eine weitere Gruppe für Erzieher
- Kinder wählen eigenes Fantasie-Profilbild
- Stimmung per Piktogramm
- Mitbringsel per Piktogramm + Freitext bei „Sonstiges“
- grüner Ring nach Check-in
- jeder Klick soll per Sound begleitet werden
- Tagesbegrüßung und Verabschiedung, täglicher Sprachwechsel
- Muttersprache + bis zu 2 weitere Sprachen
- Kinderprofil mit digitalem Bilderordner
- Foto aufnehmen/auswählen und nach Datum speichern
- Bilder beim Abholen im Profil zeigen
- Check-out/Abmelden
- Pädagogische Übersicht
- keine Anmeldung/Sicherung in dieser Version
- Es soll ein Kalender dauerhaft an der Seite zu sehen sein für Termine

WICHTIG:
Die Daten werden nur im Browser (localStorage) dieses Geräts gespeichert.
